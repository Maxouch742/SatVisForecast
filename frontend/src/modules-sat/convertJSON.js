...
export function convertJSON(orbit){
    ...
    tle_message += ` ${int_des_1part.padEnd(2)}${int_des_2part}${int_des_3part.padEnd(3, ' ')}`;
    ...
    tle_message += ` ${String(arg_perigee_int).padStart(3, ' ')}.${arg_perigee_flo.slice(-4)}`;
    ...
    tle_message += ` ${String(m_anomaly_int).padStart(3, ' ')}.${m_anomaly_flo.slice(-4)}`;
